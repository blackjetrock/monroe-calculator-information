...
if { [llength $argv] < 2} {
    ...
    puts "rom-reader-tool<file> to-bin <start address> <last address> "
    exit
}
...
proc write_binary_data {filename first_addr last_addr} {
    set first [expr $first_addr]
    set last  [expr $last_addr]
    
    set bindata [string range $::fullhexdata $first $last]
